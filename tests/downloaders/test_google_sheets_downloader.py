...
class TestGoogleSheetsDownloader:
	...
	def test_get_export_url_xlsx(self):
		...
		expected = f"https://docs.google.com/spreadsheets/d/{sheet_id}/export?format=xlsx"
		assert url == expected
	...
	@patch('src.gsparse.downloaders.google_sheets_downloader.GoogleSheetsDownloader.is_valid_google_sheets_url')
	@patch('src.gsparse.downloaders.google_sheets_downloader.GoogleSheetsDownloader.extract_sheet_id')
	@patch('src.gsparse.downloaders.google_sheets_downloader.GoogleSheetsDownloader.get_export_url')
	def test_download_sheet_success(self, mock_get_export_url, mock_extract_sheet_id, mock_is_valid):
		...
		downloader.session.get.assert_called_once_with("https://example.com/export", timeout=30)
	...
	@patch('src.gsparse.downloaders.google_sheets_downloader.GoogleSheetsDownloader.extract_sheet_id')
	@patch('src.gsparse.downloaders.google_sheets_downloader.GoogleSheetsDownloader.get_export_url')
	def test_download_sheet_request_error(self, mock_get_export_url, mock_extract_sheet_id, mock_is_valid):
	...
	def test_get_sheet_info_success(self):
		...
		with patch.object(downloader, 'is_valid_google_sheets_url', return_value=True):
			with patch.object(downloader, 'extract_sheet_id', return_value="1BxiMVs0XRA5nFMdKvBdBZjgmUUqptlbs74OgvE2upms"):
				info = downloader.get_sheet_info(url)
				
				assert info["sheet_id"] == "1BxiMVs0XRA5nFMdKvBdBZjgmUUqptlbs74OgvE2upms"
				assert info["url"] == url
	...
	def test_list_worksheets_error(self):
		...
		with patch.object(downloader, 'download_sheet', side_effect=Exception("Download error")):
			worksheets = downloader.list_worksheets(url)
